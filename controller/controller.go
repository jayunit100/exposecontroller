...
func NewController(
	...
	c.podLister, c.podController = framework.NewInformer(
		&cache.ListWatch{
			ListFunc:  podsListFunc(c.client, namespace),
			WatchFunc: podsWatchFunc(c.client, namespace),
		},
		&api.Pod{},
		resyncPeriod,
		framework.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(oldObj interface{}, newObj interface{}) {
				pod := newObj.(*api.Pod)
				if true {
					updateRelatedResources(kubeClient, oc, pod, config, authorizeURL)
				} else {
					if true {
...
func updateRelatedResources(c *client.Client, oc *oclient.Client, svc *api.Pod, config *Config, authorizeURL string) {
	// [jay] pod mutations will go here ?
...
func (c *Controller) Stop() {
	glog.Infof("stopping pod controller")
